    ...
    <div className={`flex ${isUser ? "justify-end" : "justify-start"}`}>
      <div
        className={`max-w-[85%] rounded-[var(--radius-control)] px-3.5 py-2.5 text-sm leading-relaxed whitespace-pre-wrap ${
          isUser ? "text-white" : "bg-navy-light text-text border border-border"
        }`}
      ...
      >
        ...
        {!message.streaming && (message.unanswered || message.refused) && message.contact && (
          <div className="mt-3 flex flex-col gap-2 border-t border-border/70 pt-3">
            <a
              href={message.contact.phoneHref}
              className="inline-flex h-11 items-center justify-center rounded-[var(--radius-control)] px-3 text-sm font-medium text-white"
              style={{ backgroundColor: accentColor }}
            >
              電話で問い合わせる（{message.contact.phone}）
            </a>
            <a
              href={message.contact.contactUrl}
              target="_blank"
              rel="noreferrer"
              className="inline-flex h-11 items-center justify-center rounded-[var(--radius-control)] border px-3 text-sm font-medium"
